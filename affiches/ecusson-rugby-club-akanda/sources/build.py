...
P=json.load(open('player.json'))
x0,y0,x1,y1=P['box']; S=P['S']
im=Image.open('player.png'); im=im.resize((im.width*2//3, im.height*2//3), Image.LANCZOS)
buf=io.BytesIO(); im.save(buf,'WEBP',quality=90,method=6); b64=base64.b64encode(buf.getvalue()).decode()
...
svg=f'''<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 400 480" width="400" height="480">
  ...
  <g clip-path="url(#inner)">
    ...
    <!-- Joueur -->
    <g fill="#ffffff">{ol}</g>
    <image x="{PX}" y="{PY}" width="{W:.2f}" height="{H:.2f}" xlink:href="data:image/webp;base64,{b64}"/>
  </g>
...
open('/home/user/stock/affiches/ecusson-rugby-club-akanda/ecusson-v2.svg','w').write(svg)
